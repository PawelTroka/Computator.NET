...
namespace Computator.NET.Config
{
    internal class SimpleLogger
    {
        ...
        public void Log(string message, ErrorType errorType, Exception ex)
        {
            ...
            if (!Directory.Exists(GlobalConfig.LogsDirectory))
                Directory.CreateDirectory(GlobalConfig.LogsDirectory);

            var sw = new StreamWriter(Path.Combine(GlobalConfig.LogsDirectory, fileName));
